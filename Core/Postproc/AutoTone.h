			...
			bool Apply(RadianceBuffer *p_pInput, RadianceBuffer *p_pOutput, int p_nRegionX, int p_nRegionY, int p_nRegionWidth, int p_nRegionHeight)
			{
				...
				for (int y = p_nRegionY; y < p_nRegionHeight; ++y)
				{
					for (int x = p_nRegionX; x < p_nRegionWidth; ++x)
					{
						...
						/*
						Lw[0] += Maths::Log(pInputContext->Final[0] + Maths::Epsilon);
						Lw[1] += Maths::Log(pInputContext->Final[1] + Maths::Epsilon);
						Lw[2] += Maths::Log(pInputContext->Final[2] + Maths::Epsilon);
						*/
					}
				}

				/*
				Lw[0] = Maths::Exp(1.f / p_pInput->GetArea() * Lw[0]);
				Lw[1] = Maths::Exp(1.f / p_pInput->GetArea() * Lw[1]);
				Lw[2] = Maths::Exp(1.f / p_pInput->GetArea() * Lw[2]);
				*/
				...
				for (int y = p_nRegionY; y < p_nRegionHeight; ++y)
				{
					for (int x = p_nRegionX; x < p_nRegionWidth; ++x)
					{
						...
						// pOutputContext->Final = ((pInputContext->Final - low) * (Lw * 0.18f));// / range;
						// pOutputContext->Final = ((pInputContext->Final - low) * (Lw * 0.18f));// / range;
						//pOutputContext->Final[0] = Maths::Log(Maths::Min(0.005f, Maths::Max(1.0f, pOutputContext->Final[0]))) / 5.3f + 1.f;
						//pOutputContext->Final[1] = Maths::Log(Maths::Min(0.005f, Maths::Max(1.0f, pOutputContext->Final[1]))) / 5.3f + 1.f;
						//pOutputContext->Final[2] = Maths::Log(Maths::Min(0.005f, Maths::Max(1.0f, pOutputContext->Final[2]))) / 5.3f + 1.f;
